...
class MailingGroupTask:
    ...
    def execute(self):
        for user_id in self.task.users:
            ...
            self.result[f'{user.first_name} {user.last_name}'] = {"email": None,
                                                                  "tg": {}}
            ...
            tg_notes = [{"usertype": tg_note.usertype,
                        ...
                        Telegram.objects.filter(user__id=user_id,
                                                usertype__in=self.task.users[user_id]["tg"])]
            for tg_note in tg_notes:
                self._send_telegram_message(tg_note,
                                            self.result[f'{user.first_name} {user.last_name}'])
            if self.task.users[user_id].get("email"):
                self.result[f'{user.first_name} {user.last_name}']['email'] = {}
                self._send_email(user, self.result[f'{user.first_name} {user.last_name}'])
        self._set_result()
    ...
    def _send_email(self, user: NewUser, result_dict):
        for message in self.task.messages:
            try:
                email = user.email if user.email and len(user.email) > 0 else None
                if not email:
                    result_dict["email"]["status"] = "error"
                    result_dict["email"]["errors"] = ["Email отсутствует"]
                    self.result['info']['errors'] += 1
                    return
                send_mail(message.get("theme") if message.get("theme") != "" else "Без темы",
                          message.get("text"),
                          EMAIL_HOST_USER,
                          [email],
                          fail_silently=False)
