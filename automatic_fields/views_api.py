...
class AutomaticFieldAPIView(APIView):
    def get_learning_plan_name(self):
        ...
        if qp.get('teacher'):
            ...
            return Response({'name': f'Учебный план {count + 1}'}, status=status.HTTP_200_OK)
        else:
    ...
    def get_learning_phase_name(self):
        ...
        if qp.get('learning_plan'):
            try:
                ...
                return Response({'name': f'Этап {count + 1}'}, status=status.HTTP_200_OK)
            except LearningPlan.DoesNotExist:
                return Response({'learning_plan': 'not found'}, status=status.HTTP_404_NOT_FOUND)
        else:
            return Response({'learning_plan': 'not found'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_lesson_name(self):
        ...
        if qp.get('learning_phase'):
            try:
                ...
                return Response({'name': f'Занятие {count + 1}'}, status=status.HTTP_200_OK)
            except LearningPhases.DoesNotExist:
                return Response({'learning_phase': 'not found'}, status=status.HTTP_404_NOT_FOUND)
        else:
            return Response({'learning_phase': 'not found'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_hw_name(self):
        ...
        if qp.get('lesson'):
            try:
                ...
                return Response({'name': f'ДЗ к занятию "{lesson.name}" {count + 1}'}, status=status.HTTP_200_OK)
            except Lesson.DoesNotExist:
                return Response({'lesson': 'not found'}, status=status.HTTP_404_NOT_FOUND)
        else:
            count = Homework.objects.count()
            return Response({'name': f'Домашнее задание {count+1}',
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({'data': 'uncorrected data'}, status=status.HTTP_400_BAD_REQUEST)