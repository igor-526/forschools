...
class ChannelMessage:
    ...
    def __str__(self):
        text = self.get_text_without_hashtags() \
            if self.get_text_without_hashtags() else "отсутствует"
        files = self.file_info if self.file_info else "отсутствуют"
...
def get_dialog(dialogs: TotalList):
    out = list(filter(
        lambda d: d.name == settings.TGPARSER_CHANNEL_NAME, dialogs
    ))
    return out[0]


async def get_m_owner():
    usrID = input("Введите ID пользователя, "
                  "который будет владельцем материалов: ")
...
def log_result(saved: int | None = None,
...
def parse_manual_set_cats(obj=None):
    categories = []
    while True:
        new_cat = input("Введите категории, в которые будем добавлять, "
                        "после чего нажмите Enter на пустой строке: ")
        if new_cat != "":
        ...
        while action != "q":
            with open("tgparser/logs/saved.txt", "r", encoding="utf-8") as f:
                if str(msg.id) in [e.strip("\n") for e in f.readlines()]:
                    break
            if (not msg_object.file_info and
                    not msg_object.category and
                    not msg_object.get_text_without_hashtags()):
                break
