...
def notification_lessons_soon() -> None:
    def notify_listeners_telegram(lesson_: Lesson,
        ...
        for listener in listeners_:
            telegrams = Telegram.objects.filter(
                Q(allowed_users=listener) | Q(allowed_parents=listener)
            ).values(
                "tg_id", "usertype",
    ...
    def notify_teacher_telegram(lesson_: Lesson,
                                listeners_: QuerySet,
                                teacher_: NewUser) -> None:
        listeners__names_text = ', '.join(
            [f'<b>{listener.first_name} {listener.last_name}</b>'
             for listener in listeners_]
        )
        message_text = (
    ...
    def notify_telegram(lessons_info_: dict[NewUser: dict]) -> None:
        for teacher in lessons_info_:
            ...
            for lesson in lessons_info_[teacher][:15]:
                listeners_names_text = ', '.join(
                    [f'{listener.first_name} {listener.last_name}'
                     for listener in lesson['listeners']]
                )
                message_text += (
                    f"{lesson['name']} "
                    f"({listeners_names_text})"
...
class LessonsDataCreator:
    ...
    def get_field_review_dt_value(self, lesson: Lesson) -> str:
        if (lesson.lesson_teacher_review is None or
            ...
            return "Отсутствует"
        review_dt = lesson.lesson_teacher_review.dt.strftime('%d.%m.%Y %H:%M')
        return f"Дата и время заполнения: {review_dt}"
