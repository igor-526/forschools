...
class UserLoginMAAPIView(APIView):
    def post(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            return Response(request.POST.get('redirect_url') if
                            request.POST.get('redirect_url') else '/ma/',
                            status=status.HTTP_302_FOUND)
        if request.POST:
            tg_id = request.POST.get('tg_id')
            if not tg_id:
                return Response(data="/ma/error/login_not_tg_id/",
                                status=status.HTTP_302_FOUND)
            try:
                ...
                return Response(data="/ma/error/login_not_tg_note/",
                                status=status.HTTP_302_FOUND)
