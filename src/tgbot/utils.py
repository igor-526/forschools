...
class AsyncClass:
    async def send_tg_message_sync(self, tg_id, message,
    ...
    async def check_unsent_messages(self, notify=True):
        all_users = [
            {"tg_id": tg_note.tg_id,
             "full_name": f'{tg_note.user.first_name} '
    ...
    async def add_hw(
        ...
        try:
            ...
            listeners_str = ', '.join([f'{listener.first_name} '
                                       f'{listener.last_name}'
                                       for listener in listeners])
            msg_text = (
                f"Вы задаёте ДЗ к занятию от {lesson.date.strftime('%d.%m')}\n"
                f"{'Ученик: ' if len(listeners) == 1 else 'Ученики: '}: "
                f"{listeners_str}"
            )
            if plan.pre_hw_comment:
                msg_text += (f'\n\n<b>Необходимо учесть '
                             f'следующий комментарий:</b>'
                             f'\n{plan.pre_hw_comment}')
            msg_text += ("\n\nПерешлите сюда или прикрепите "
                         "материал, или напишите сообщение\n"
    ...
    async def send_lesson_place(
            self, lesson_id, tg_id
    ) -> Dict[str, bool] | Dict[str, str | bool]:
        try:
...
async def get_user(tg_id) -> NewUser | None:
    tg_note = await Telegram.objects.filter(
        tg_id=tg_id).select_related("user").afirst()
    if tg_note:
        return tg_note.user
    else:
        return None
...
def send_materials(initiator: NewUser,
                   recipient: NewUser,
                   materials,
                   sendtype):
    ...
    for user_tg in user_tgs:
        msg_result = sync_funcs.send_tg_message_sync(
            tg_id=user_tg.get("tg_id"),
...
def send_homework_tg(initiator: NewUser,
                     listener: NewUser,
...
def send_homework_answer_tg(user: NewUser,
                            homework: Homework,
                            status: int) -> dict:
    ...
    recipient = None
    if status == 3:
        msg = (f"Пришёл новый ответ от ученика по "
               f"ДЗ <b>'{homework.name}'</b>")
        initiator = homework.listener
        recipient = homework.teacher
    elif status in [4, 5]:
        msg = (f"Пришёл новый ответ от преподавателя "
               f"по ДЗ <b>'{homework.name}'</b>")
...
def notify_group_chat_message(message: Message):
    def notify(tg_ids, text_type="user"):
        msg_text = ""
        if text_type == "user":
            msg_text = (f'<b>Новое сообщение из "{group.name}"'
                        f'</b>\n{message.message}')
        elif text_type == "receiver_parent":
            msg_text = (f'<b>Новое сообщение ДЛЯ УЧЕНИКА из '
                        f'"{group.name}"</b>\n{message.message}')
        elif text_type == "sender_parent":
            msg_text = (f'<b>Новое сообщение ОТ УЧЕНИКА в '
                        f'"{group.name}"</b>\n{message.message}')
    ...
    receiver_parents_tg_id = [
        tgnote.tg_id for tgnote in
        Telegram.objects.filter(
            user__in=group.users.exclude(id=message.sender.id)
        )
        .exclude(Q(usertype="main") | Q(tg_id__in=users_tg_id))
    ]
    sender_parents_tg_id = [tgnote.tg_id for tgnote in
                            message.sender.telegram.exclude(usertype="main")]
    receivers_tg_id = [
        tgnote.tg_id for tgnote in
        Telegram.objects.filter(
            user__in=group.users.exclude(id=message.sender.id), usertype="main"
        )
    ]
...
def notification_log_journal(recipient: NewUser | int,
                             event: int,
                             result_status: str,
                             msg_text: str | None,
                             msg_id: int | None,
                             usertype: str | None,
                             errors: list | None = None) -> TgBotJournal:
    if errors is None:
        errors = []
