...
async def f_homework_agr_message(callback: CallbackQuery,
    ...
    if callback_data.action == "agreement_accept":
        msg = ("При необходимости отправьте мне комментарий на "
        ...
        msg = ("Отправьте мне хотя бы один комментарий на "
               "действие преподавателя.\n"
...
async def f_homework_agr_send(tg_id: int,
    ...
    async def get_logs_info():
        hw = await (Homework.objects
                    .select_related("teacher", "listener")
                    .aget(pk=st_data.get("hw_id")))
        last_log = await hw.aget_status()
        if last_log and last_log.status == 7:
            hw_group = await hw.homeworkgroups_set.afirst()
            hws = [hw async for hw in
                   hw_group.homeworks
                   .select_related("teacher", "listener").all()] \
                if hw_group else [hw]
        else:
            hws = [hw]
        ...
        to_agreement = [log async for log in
                        HomeworkLog.objects
                        .select_related("homework", "homework__listener",
    ...
    async def notify_users(logs, hws, action):
        ...
        is_teacher = logs[0].user == (logs_info.get("plan").teacher or
                                      logs[0].user ==
                                      logs_info.get("plan").default_hw_teacher)
        is_curator = await logs_info.get("plan").curators.filter(
            id=logs[0].user.id).aexists()
        for log_ in logs:
            if action == "agreement_accept":
                if log_.status == 7:
                    msg_listener = "У вас новое домашнее задание!"
                    if is_teacher:
                        msg_teacher = ("ДЗ согласовано методистом и "
                                       "задано ученику")
                        msg_curator = "Преподаватель задал новое ДЗ"
                    elif is_curator:
                        msg_teacher = ("ДЗ куратора согласовано "
                                       "методистом и задано ученику")
                elif log_.status == 4:
                    msg_listener = "Домашнее задание принято!"
                    if is_teacher:
                        msg_teacher = ("Принятие ДЗ согласовано методистом. "
                                       "Ученик уведомлён")
                        msg_curator = "Преподаватель принял у ученика ДЗ"
                    elif is_curator:
                        msg_teacher = ("Принятие ДЗ от куратора согласовано "
                                       "методистом. Ученик уведомлён")
                elif log_.status == 5:
                    msg_listener = "Домашнее задание отправлено на доработку!"
                    if is_teacher:
                        msg_teacher = ("Отправка ДЗ на доработку согласована "
                                       "методистом. Ученик уведомлён")
                        msg_curator = "Преподаватель отправил на доработку ДЗ"
                    elif is_curator:
                        msg_teacher = ("Отправка ДЗ от куратора на доработку "
                                       "согласована методистом. "
                                       "Ученик уведомлён")
                ...
                if log_.status == 7:
                    ...
                    msg_teacher = ("Отправка ДЗ на доработку "
                                   "НЕ согласовано методистом")
    ...
    async def journal_log():
        title = "Действие преподавателя согласовано" \
            if st_data.get("action") == "agreement_accept" else \
            "Действие преподавателя НЕ согласовано"
        color = "info" if st_data.get("action") == "agreement_accept" \
            else "warning"
        lesson = await logs_info.get("homeworks")[0].lesson_set.afirst()
        content = {
            "list": [],
            "text": [
                f'Методист обработал действие преподавателя по ДЗ к занятию '
                f'"{lesson.name}" от {lesson.date.strftime("%d.%m.%Y")}',
                f'Проверяющий ДЗ: '
                f'{logs_info.get("homeworks")[0].teacher.first_name} '
        ...
        for hw in logs_info.get("homeworks"):
            ...
            buttons.append({"inner": f"{hw.name} ({hw.listener.first_name} "
                                     f"{hw.listener.last_name})",
        ...
        if st_data.get("action") == "agreement_decline":
            buttons.append(
                {"inner": '<i class="bi bi-chat"></i>',
                 "href": f"#tag=hw"
                         f"{logs_info.get('homeworks')[0].id}"}
            )
    ...
    if st_data.get("action") == "agreement_accept":
        ...
        answer_msg = ("Действия преподавателя согласованы. "
                      "Уведомления отправлены")
        if logs_info.get("homeworks") and len(logs_info.get("homeworks")) > 1:
            answer_msg += ("\nНет необходимости согласовывать "
                           "другие ДЗ для группового занятия")
    elif st_data.get("action") == "agreement_decline":
        agreement["accepted"] = False
        agreement["comment"] = "\n".join(st_data.get("comment"))
        answer_msg = ("Действия преподавателя не согласованы. "
                      "Уведомления отправлены")
        if logs_info.get("homeworks") and len(logs_info.get("homeworks")) > 1:
            answer_msg += ("\nНет необходимости согласовывать "
                           "другие ДЗ для группового занятия")
    for log in logs_info.get("logs"):
        ...
        await log.asave()
    await notify_users(logs_info.get("logs"),
                       logs_info.get("homeworks"),
                       st_data.get("action"))
    await journal_log()
    await bot.send_message(chat_id=tg_id,
                           text=answer_msg)
