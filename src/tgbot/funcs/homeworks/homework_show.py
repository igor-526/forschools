...
from tgbot.utils import get_tg_note, get_user
...
class TGHomework:
        ...
        self.user_roles = {
            ...
            "is_curator": (self.homework.for_curator and
                           ...
                           (await self.plan.curators.filter(
                               id=self.user.id).aexists()))
        }
        self.homework_last_log = await self.homework.aget_status(True) \
            if self.user_roles.get("is_listener") \
            else await self.homework.aget_status(False)

    def init_homework(self) -> None:
        self.homework = Homework.objects.select_related(
            "listener", "teacher").get(pk=self.homework_id)
        self.telegram_note = Telegram.objects.select_related(
            "user").get(tg_id=self.telegram_id)
        ...
        self.user_roles = {
            ...
            "is_curator": (self.homework.for_curator and
                           self.plan and
                           (self.plan.curators.filter(
                               id=self.user.id).exists()))
        }
        self.homework_last_log = self.homework.get_status(accepted_only=True) \
            if self.user_roles.get("is_listener") \
            else self.homework.get_status(accepted_only=False)

    async def send_materials(self) -> None:
        materials = [_ async for _ in
                     self.homework.materials.all().order_by("uploaded_at")]
        if materials:
            delete_settings = None if self.user_roles["is_listener"] \
                else {"action": "hw", "id": self.homework_id}
            for material in materials:
                await send_material_item(self.telegram_id,
                                         material,
                                         meta=False,
    ...
    async def _show_log_item(self, log_id: int) -> None:
        async def get_edit_perm() -> bool:
            ...
            if log.user == self.user:
                return True
            return True if self.plan and self.plan.metodist == self.user \
                else False
        ...
        async def send_files() -> None:
            ...
            if len(files) > 0:
                for file in files:
                    rm = get_hw_log_delete_file_button(log.id, file.id) \
                        if edit_perm else None
                    await send_file(self.telegram_id, file, rm)

        log = await HomeworkLog.objects.select_related(
            "user", "homework").aget(pk=log_id)
        comment = log.comment.replace('<br>', '\n') if log.comment else '-'
        edit_perm = await get_edit_perm()
        rm_text = get_hw_log_edit_button(log.id) if edit_perm else None
        msg = (
            f"<b>{log.user}: {status_code_to_string(log.status)}</b> - "
            f"{log.dt.astimezone().strftime('%d.%m %H:%M')}\n\n"
            f"{comment}\n")
    ...
    async def send_last_log(self) -> None:
        if self.user_roles["is_listener"]:
            ...
            logs = [{"status": log.status,
                     "id": log.id} async for
                    log in self.homework.log.order_by("-dt")]
        last_logs = []
        for log in logs:
            if last_logs:
        ...
        for log in [l_log["id"] for l_log in last_logs]:
    ...
    async def send_actions(self, materials_button) -> None:
        async def get_check_button() -> bool:
            ...
            if self.user_roles.get("is_teacher"):
                return True
            if self.plan:
                return (self.user_roles.get("is_methodist")
                        or self.user_roles.get("is_curator"))
            return False

        async def get_agreement_buttons() -> bool:
            if self.user_roles.get("is_methodist"):
                return self.homework_last_log.agreement.get(
                    "accepted") is False
            else:
                return False
        ...
        send_button = (self.homework_last_log.status in [2, 3, 5, 7]
                       and self.user_roles.get("is_listener"))
        agreement_buttons = await get_agreement_buttons()
        check_button = False if agreement_buttons \
            else (await get_check_button())
    ...
    async def add_materials(self, state: FSMContext) -> None:
        await state.set_data({
            "new_hw": {
                ...
                "materials": [m.id async for m in
                              self.homework.materials.all()],
                "deadline": {
        ...
        await bot.send_message(
            ...
            text="Перешлите сюда или прикрепите материал, "
