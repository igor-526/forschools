...
from homework.models import Homework, HomeworkGroups, HomeworkLog
...
from tgbot.keyboards.homework import (get_homework_add_ready_buttons,
                                      get_homework_curator_button,
                                      get_homework_editing_buttons,
                                      get_homework_lessons_buttons,
                                      get_homework_menu_buttons,
...
async def add_homework_select_lesson(
        user_tg_id: int,
    ...
    date = datetime.date.today() if not date_ \
        else datetime.datetime.strptime(date_, "%d.%m.%Y").date()
    prev_date = date - datetime.timedelta(days=1)
    next_date = (date + datetime.timedelta(days=1)) \
        if date != datetime.date.today() else None
    prev_date = {
    ...
    if message:
        await message.reply(text="К какому занятию прикрепить ДЗ?",
        ...
        await callback.message.edit_text(
            text="К какому занятию прикрепить ДЗ?",
            reply_markup=get_homework_lessons_buttons(
                lessons,
                prev_date,
                date.strftime("%d.%m.%Y"),
...
async def add_homework_set_homework_ready(
        state: FSMContext,
        message: types.Message = None,
        callback: types.CallbackQuery = None
):
    async def new_hw_result():
        ...
        is_teacher = user in [teacher, lesson.replace_teacher,
                              plan.teacher if plan else None]
        ...
        if lesson.status == 3:
            msg_text = ("Вы не можете задать ДЗ к этому занятию, "
                        "так как оно отменено")
        elif is_teacher:
            if lesson.status == 1 and methodist:
                msg_text = (f"ДЗ для {listener.first_name} "
                            f"{listener.last_name} отправлено на согласование")
                await hw.aset_assigned()
                rm = get_homework_add_ready_buttons(
                    tg_note=tg_note,
                    hw_id=hw.id,
                    lesson_id=None,
                ...
                msg_text = (f"ДЗ для {listener.first_name} "
                            ...
                            f"сохранено и будет отправлено на "
                            f"согласование методисту после "
            ...
            elif (lesson.status == 0 and
                  lesson_can_be_passed and
                  methodist is None):
                msg_text = (f"ДЗ для {listener.first_name} "
                            f"{listener.last_name} "
                            f"сохранено и будет задано после "
                            f"заполнения формы занятия")
                rm = get_homework_add_ready_buttons(
                    tg_note=tg_note,
                    hw_id=hw.id,
            ...
            if lesson.status == 1 and methodist:
                ...
                await homework_tg_notify(
                    ...
                    methodist.id,
                    [hw],
                    "Куратор задал ДЗ. Требуется согласование", 8
                )
            elif lesson.status == 1 and methodist is None:
                msg_text = (f"ДЗ для {listener.first_name} "
                            f"{listener.last_name} задано")
                ...
                msg_text = (
                    f"ДЗ для {listener.first_name} {listener.last_name} "
                    f"не может быть задано, так как занятие не"
            ...
            if lesson.status == 1:
                ...
                msg_text = (f"ДЗ для {listener.first_name} "
                            f"{listener.last_name} "
                            f"создано и будет задано после "
                            f"проведения занятия")
                rm = get_homework_add_ready_buttons(
                    tg_note=tg_note,
                    hw_id=hw.id,
    ...
    async def notify_hw_changed():
        ...
        if (st.status in [2, 3, 5] and
                st.agreement.get("accepted") in [True, None]):
            await homework_tg_notify(user,
                                     hw.listener.id,
                                     [hw],
                                     "Домашнее задание было изменено")
        if (st.agreement.get("accepted") is not None and
                not st.agreement.get("accepted")):
            ...
            await homework_tg_notify(
                ...
                f"Преподаватель: {hw.teacher.first_name} "
                f"{hw.teacher.last_name}\n"
    ...
    async def log_add_materials(old_mat=None, new_mat=None):
        ...
        if old_mat is None:
            old_mat = []
        added_materials = list(filter(
            lambda mat_id: mat_id not in old_mat, new_mat
        ))
        if added_materials:
    ...
    if hw_id:
        homework = await Homework.objects.select_related(
            "teacher", "listener").aget(id=hw_id)
        hw_group = await homework.homeworkgroups_set.afirst()
        if hw_group:
            hws = [
                _ async for _ in
                hw_group.homeworks.select_related("teacher", "listener").all()
            ]
        else:
            hws = [homework]
        ...
        for hw in hws:
            ...
            hw.description = statedata.get("new_hw").get("description") \
                if statedata.get("new_hw").get("description") \
                else "-"
            ...
            await log_add_materials(old_materials,
                                    statedata.get("new_hw").get("materials"))
            await notify_hw_changed()
        await message.answer(text="ДЗ успешно изменено",
                             reply_markup=None)
        await send_menu(message.from_user.id, state)
    else:
        lesson = await Lesson.objects.select_related("replace_teacher").aget(
            pk=statedata.get("new_hw").get("lesson_id")
        )
        listeners = await lesson.aget_listeners()
        plan = await lesson.aget_learning_plan() if lesson else None
        ...
        for listener in listeners:
            hw = await Homework.objects.acreate(
                ...
                description=statedata.get("new_hw").get("description")
                if statedata.get("new_hw").get("description") else "-",
            ...
            hw = await Homework.objects.select_related(
                "teacher", "listener").aget(pk=hw.id)
            await hw.materials.aset(statedata.get("new_hw").get("materials"))
            if lesson:
    ...
    if func == "complete":
        if 'Listener' in groups:
            homeworks = list(filter(
                lambda hw: hw['hw_status'] in [7, 2, 3, 5],
                [{
                    'obj': hw,
            ...
            if len(homeworks) == 0:
                ...
                await bot.send_message(
                    chat_id=tg_id,
                    text="Вот домашние задания, "
                         "которые ждут Вашего выполнения:",
        ...
        if 'Metodist' in groups:
            homeworks = list(filter(
                lambda hw: hw['hw_status'] is not None and not hw['hw_status'],
                [{
                    'obj': hw,
                    'hw_status':
                        (await hw.aget_status()).agreement.get("accepted"),
                    'name': await hw.aget_tg_name(groups)
                } async for hw in
                    Homework.objects.select_related("listener").filter(
        ...
        if 'Teacher' in groups:
            homeworks = list(filter(
                lambda hw: hw['hw_status'].status in [3, 5] and (
        ...
        if 'Curator' in groups:
            homeworks = list(filter(
                lambda hw: hw['hw_status'].status in [3, 5],
                [{
                    'obj': hw,
                    'hw_status': await hw.aget_status(),
                    'name': await hw.aget_tg_name(groups)
...
async def send_hw_answer(callback: CallbackQuery,
    ...
    if 'Listener' in groups and hw_status.status in [7, 2, 3, 5]:
        await bot.send_message(
            ...
            text="Отправьте мне сообщения, "
                 "содержащие решение домашнего задания, "
...
async def send_hw_check(callback: CallbackQuery,
    ...
    if await get_access():
        await bot.send_message(
            chat_id=callback.from_user.id,
            text="Отправьте мне сообщения, содержащие "
...
async def hw_send(tg_id: int, state: FSMContext):
    ...
    async def notify_teacher(msg="Пришёл новый ответ от ученика по ДЗ"):
        ...
        if teacher_tg:
            try:
                msg_object = await bot.send_message(
                    chat_id=teacher_tg,
                    text=msg,
                    reply_markup=get_homeworks_buttons([{
    ...
    async def notify_methodist(msg="Требуется проверка "
                                   "действия преподавателя"):
        methodist_tg = await get_tg_id(lp.metodist.id, True)
        if methodist_tg:
            try:
                msg_object = await bot.send_message(
                    chat_id=methodist_tg,
                    text=msg,
                    reply_markup=get_homeworks_buttons([{
    ...
    if not state_data['files'] and not state_data['comment']:
        await bot.send_message(
            chat_id=tg_id,
            text="Вы не можете отправить пустой ответ. "
                 "Пожалуйста, пришлите мне текст, фотографии, "
    ...
    if hw_action == 'send':
        if is_listener:
            ...
            if hw.for_curator and (lp and not await lp.curators.filter(
                    id=hw.teacher.id).aexists()):
        ...
        if is_methodist:
            ...
            if hw_action == 'check_accept':
                ...
                await notify_teacher(
                    "Методист принял домашнее задание"
                )
            elif hw_action == 'check_revision':
                await notify_listener(
                ...
                await notify_teacher(
                    "Методист отправил домашнее задание на доработку"
                )
        elif is_teacher:
            if lp and lp.metodist:
                ...
                if hw_action == 'check_accept':
                    await notify_methodist(
                        "Преподаватель принимает ДЗ. Требуется согласование"
                    )
                elif hw_action == 'check_revision':
                    await notify_methodist(
                        "Преподаватель отправляет ДЗ на доработку. "
                ...
                if hw_action == 'check_accept':
                    ...
                    if hw.for_curator:
                        await notify_curators(
                            "Преподаватель принял домашнее задание"
                        )
                elif hw_action == 'check_revision':
                    await notify_listener(
                        "Домашнее задание отправлено на доработку"
                    ...
                    if hw.for_curator:
                        await notify_curators(
                            "Преподаватель отправил на "
                            "доработку домашнее задание"
                        )
        elif is_curator:
            if lp and lp.metodist:
                await get_hw_log_object(True)
                ...
                if hw_action == 'check_accept':
                    await notify_methodist(
                        "Куратор принимает ДЗ. Требуется согласование"
                    )
                elif hw_action == 'check_revision':
                    await notify_methodist(
                        "Куратор отправляет ДЗ на доработку. "
                        "Требуется согласование"
                    )
            else:
                await get_hw_log_object(False)
                ...
                if hw_action == 'check_accept':
                    ...
                    if hw.for_curator:
                        await notify_curators(
                            "Куратор принял домашнее задание"
                        )
                elif hw_action == 'check_revision':
                    await notify_listener(
                        "Домашнее задание отправлено на доработку"
                    ...
                    if hw.for_curator:
                        await notify_curators(
                            "Куратор отправил на доработку домашнее задание"
                        )
        else:
            await bot.send_message(
                chat_id=tg_id,
...
async def homework_tg_notify(initiator: NewUser,
                             recipient_user_id: int,
                             homeworks: list[Homework],
                             text="У вас новые домашние задания!",
                             log_event=3):
    recipients_tgs = await get_tg_id(recipient_user_id)
    user_groups = [group.name async for group in
                   (await NewUser.objects.aget(
...
async def hw_for_curator_set(callback: CallbackQuery,
    ...
    await hw.asave()
    await callback.answer("Кураторы теперь могут работать с этим ДЗ"
                          if hw.for_curator else
                          "Кураторы больше не смогут работать с этим ДЗ")
    await callback.message.edit_text(
        text=callback.message.text,
        reply_markup=get_homework_curator_button(
            callback_data.hw_id, hw.for_curator)
    )
