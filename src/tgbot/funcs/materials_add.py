...
class FileParser:
    ...
    def set_file_data(self):
        ...
        if self.material_message.text:
            self.file_type = "text"
            self.file_description = self.material_message.text
            self.file_name = " ".join(self.material_message.text.split(
                " ")[:10])
            self.file_format = "txt"

        if self.material_message.voice:
            self.file_type = "voice"
            self.file_name = (f'Голосовое сообщение TG '
        ...
        if self.material_message.photo:
            ...
            self.file_name = (f'Изображение TG '
                              f'{self.material_message.message_id}')
        ...
        if self.material_message.audio:
            ...
            self.file_name = tg_file_name.split(".")[0] \
                if tg_file_name else (f'Аудиозапись TG '
                                      f'{self.material_message.message_id}')
        ...
        if self.material_message.animation:
            ...
            self.file_format = (
                self.material_message.animation.file_name.split(".")[-1]
        ...
        if self.material_message.document:
            if not self.material_message.animation:
                file_type = get_type_by_ext(
                    self.material_message.document.file_name.split(".")[-1]
                )
        ...
        if self.material_message.video:
            ...
            self.file_name = file_name_tg.split(".")[0] \
                if file_name_tg else (f'Видео TG '
                                      f'{self.material_message.message_id}')
    ...
    async def validate_exists(self):
        if self.mode == "material":
            ...
            return
        mat = await model.objects.filter(tg_url=self.file_id_tg).afirst() \
            if self.file_id_tg else None
        if not mat:
            mat_name = await model.objects.filter(
                name=self.file_name).aexists()
            if mat_name:
                counter = 1
                while True:
                    new_name = f"{self.file_name[:190]}({counter})"
                    mat_name_exists = await model.objects.filter(
                        name=new_name).aexists()
                    if mat_name_exists:
                        counter += 1
                    else:
    ...
    async def set_path(self):
        if self.file_type == "text":
            ...
            self.file_path_db = os.path.join(
                folder_name, f'{self.file_name}.{self.file_format}'
            )
            self.file_path = os.path.join(
                MEDIA_ROOT, folder_name, f'{self.file_name}.{self.file_format}'
            )
        else:
            file = await self.material_message.bot.get_file(
                file_id=self.file_id_tg)
    ...
    async def add_material_db(self):
        try:
            ...
            await self.material_message.reply(
                **self.generate_success_message()
            )
        except Exception as e:
            await self.update_status_message(
                f'Произошла ошибка при добавлении материала:\n{e}'
            )
    ...
    async def download(self):
        try:
            ...
            self.file_owner = await get_user(
                self.material_message.from_user.id
            )
            ...
            if self.file_type == "text":
                ...
                await self.update_status_message(
                    "Определён тип голосового сообщения"
                )
                if self.mode == "material":
                    await self.add_material_db()
                elif self.mode == "file":
            ...
            if self.file_type == "photo":
                ...
                await self.update_status_message(
                    "\u2757\u2757\u2757"
                    "Данный документ не может быть загружен, так как "
