...
async def send_file(tg_id: int,
                    file_object: File,
                    reply_markup=None,
                    protect=False) -> None:
    file = file_object.tg_url if file_object.tg_url \
        else FSInputFile(path=file_object.path.path)
    ...
    if file_type == "image_formats":
        ...
        try:
            ...
            file_id = None
    elif file_type in ["pdf_formats", "archive_formats",
                       "presentation_formats", "word_formats"]:
        message = await bot.send_document(chat_id=tg_id,
                                          document=file,
                                          protect_content=protect,
                                          caption=file_object.caption,
