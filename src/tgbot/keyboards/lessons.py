...
def get_lesson_review_field_keyboard(
        state_data: dict, current: str
    ...
    if current == "confirm":
        ...
        return ReplyKeyboardMarkup(
            resize_keyboard=True,
            keyboard=[get_for_materials(),
                      [KeyboardButton(text="Отмена")]]
        )
    elif current == "lexis":
        return ReplyKeyboardMarkup(resize_keyboard=True,
                                   keyboard=[get_for_lexis(),
                                             [KeyboardButton(text="Отмена")]]
                                   )
    elif current == "grammar":
        return ReplyKeyboardMarkup(
            resize_keyboard=True,
            keyboard=[get_for_grammar(),
                      [KeyboardButton(text="Отмена")]]
        )
    elif current == "note":
        return ReplyKeyboardMarkup(
            resize_keyboard=True,
            keyboard=[get_for_note(),
                      [KeyboardButton(text="Отмена")]]
        )
    elif current == "org":
        return ReplyKeyboardMarkup(
            resize_keyboard=True,
            keyboard=[get_for_org(),
                      [KeyboardButton(text="Отмена")]]
        )
