...
def get_homework_menu_buttons(
        params: dict, tg_note: Telegram
...
def get_homework_editing_buttons() -> ReplyKeyboardMarkup:
    ...
    edit_materials_button = KeyboardButton(
        text="Показать прик. материалы"
    )
    save_hw_button = KeyboardButton(text="Подтвердить ДЗ")
    return ReplyKeyboardMarkup(resize_keyboard=True,
                               keyboard=[[save_hw_button],
...
def get_homework_lessons_buttons(lessons: list,
    ...
    builder = InlineKeyboardBuilder()
    for lsn in lessons:
        listeners_names = ", ".join([
            listener.first_name + " " + listener.last_name
            for listener in lsn.get("listeners")
        ])
        btn_text = (
            f'{("("+"X"+")") if lsn.get("status") == 0 else ""} '
            f'{lsn.get("start_time")}-{lsn.get("end_time")}: '
            f'{listeners_names}'
        )
        builder.button(
            text=btn_text,
    ...
    if next_date:
        builder.button(
            ...
            callback_data=HomeworkNewSelectDateCallback(
                date=next_date.get('callback'))
        )
        last_row_counter += 1
    sizes = [1 for _ in lessons]
    if last_row_counter:
...
def get_homework_agreement_buttons(
        action="accept"
) -> ReplyKeyboardMarkup | None:
    cancel_button = KeyboardButton(text="Отмена")
    accept_button = KeyboardButton(text="Согласовать")
    ...
    if action == "accept":
        ...
        return ReplyKeyboardMarkup(resize_keyboard=True,
                                   keyboard=[[decline_button],
                                             [cancel_button]])
    else:
        return None


def get_homework_curator_button(
        hw_id: int, for_curator_status=True
) -> InlineKeyboardMarkup:
...
def get_homework_add_ready_buttons(
        tg_note: Telegram,
        hw_id: int = None,
        lesson_id: int = None,
        for_curator_status: bool = None,
        form_review_mode: int = 0
) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    if hw_id:
        hw_url = WebPlatformUrl("homeworks")
...
def get_hw_log_delete_file_button(
        hw_log_id: int, file_id: int
) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(
...
def get_homework_notification_menu_buttons(
        tg_note: Telegram = None,
