...
def get_settings_keyboard(settings: dict) -> InlineKeyboardMarkup:
    ...
    def set_notifications_tg_connecting():
        ...
        builder.button(
            ...
            callback_data=SettingsCallback(
                action="notifications_tg_connecting"
            )
        )
    ...
    def set_notifications_lessons_email():
        ...
        builder.button(
            text=text,
            callback_data=SettingsCallback(
                action="notifications_lessons_email"
            )
        )
    ...
    def set_timezone_setting_button():
        if settings.get("timezone") is None:
            return None
        text = "Часовой пояс: "
        text += f'+{settings.get("timezone")}' \
            if settings.get("timezone") > 0 else str(settings.get("timezone"))
        builder.button(
            text=text,
...
def get_settings_timezone_keyboard(
        current: int = None
