...
@router.message(StateFilter(HomeworkFSM.send_hw_files),
                F.text == "Отправить")
async def h_homework_send_ready(message: types.Message,
                                state: FSMContext) -> None:
...
async def h_homework_keep_media_group(message: types.Message,
                                      state: FSMContext,
                                      media_events=[]):
    files_list = []
    comments_list = []
...
@router.message(StateFilter(HomeworkFSM.send_hw_files))
async def h_homework_keep_files(message: types.Message,
                                state: FSMContext) -> None:
    messages = [message]
