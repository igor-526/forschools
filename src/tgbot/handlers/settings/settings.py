...
from tgbot.funcs.settings import (generate_settings_message,
                                  ...
                                  settings_set_timezone_message,
                                  settings_switch_lesson_review,
                                  ...
                                  settings_switch_show_hw_materials)
from tgbot.keyboards.callbacks.settings import (SetTimezoneCallback,
                                                SettingsCallback)
...
@router.callback_query(SettingsCallback.filter(
    F.action == 'notifications_lesson_day'
))
async def h_settings_set_notification_lesson_day(
...
@router.callback_query(SettingsCallback.filter(
    F.action == 'notifications_lessons_hour'
))
async def h_settings_set_notification_lesson_hour(
        callback: CallbackQuery
) -> None:
...
@router.callback_query(SettingsCallback.filter(
    F.action == 'notifications_tg_connecting'))
async def h_settings_set_notification_tg_connecting(
        callback: CallbackQuery
) -> None:
...
@router.callback_query(SettingsCallback.filter(
    F.action == 'notifications_lessons_email'))
async def h_settings_set_notifications_lessons_email(
        callback: CallbackQuery
) -> None:
...
async def h_settings_set_timezone(callback: CallbackQuery,
                                  callback_data: SetTimezoneCallback) -> None:
