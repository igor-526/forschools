...
async def h_mainmenu_homeworks(message: types.Message,
...
async def h_mainmenu_platform(message: types.Message) -> None:
    await message.answer(
        text="Нажмите на кнопку для перехода на платформу",
...
async def h_mainmenu_message_media_group(message: types.Message,
                                         state: FSMContext,
                                         media_events=[]):
...
@router.message(StateFilter(None))
async def h_mainmenu_message(message: types.Message,
                             state: FSMContext) -> None:
    data = await state.get_data()
