...
from tgbot.funcs.homeworks.homework_show import (edit_homework_in_tg,
                                                 open_homework_in_tg,
                                                 send_link_to_tg)
...
from .permissions import (CanEditHomeworkAdminComment,
...
from .serializers import (HomeworkListSerializer,
                          HomeworkLogListSerializer,
                          HomeworkLogSerializer,
                          HomeworkSerializer)
...
class HomeworkListCreateAPIView(LoginRequiredMixin, ListCreateAPIView):
    ...
    def filter_queryset_all(self, queryset):
        ...
        if methodists:
            query['lesson__learningphases__learningplan__metodist__in'] = (
                methodists)
        if lesson:
            query['lesson'] = lesson
        if name:
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if "Metodist" in user_groups:
            q |= Q(
                lesson__learningphases__learningplan__metodist=(
                    self.request.user)
            )
        ...
        if "Curator" in user_groups:
            q |= Q(
                lesson__learningphases__learningplan__curators=(
                    self.request.user)
            )
...
class HomeworkItemPageSendTelegramAPIView(LoginRequiredMixin, APIView):
    def post(self, request: Request, pk: int, *args, **kwargs):
        ...
        try:
            ...
            user = request.user
        user_telegrams = user.telegram_allowed_user.filter(
            user=user
        ).values_list("tg_id", flat=True)
        if not user_telegrams:
            return Response(
                data={"error": "Нет ни одного авторизованного "
                               "под этим пользователем Telegram"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if method == "edit":
            ...
            for tg_id in user_telegrams:
                if method == "open":
                    ...
                    send_link_to_tg(tg_id, homework.id,
                                    request.POST.get("message"))
                else:
                    return Response(data={
                        "error": "Не обнаружен способ открытия ДЗ"
                    },
                        status=status.HTTP_400_BAD_REQUEST)
        return Response(data={"status": "ok"},
                        status=status.HTTP_200_OK)
    ...
    def get_files(self):
        ...
        for file in self.request.data.getlist('files'):
            f = File.objects.create(
                ...
                is_animation=get_type_by_ext(file.name.split(".")[-1]) == (
                    "animation_formats"),
                path=file
            )
            result.append(f.id)
        ...
        user_log = {
            ...
            "content": {"list": [],
                        "text": [
                            ...
                            f'Проверяющий ДЗ: '
                            f'{homeworks_[0].teacher.first_name} '
