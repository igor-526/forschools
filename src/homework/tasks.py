...
from tgbot.utils import (notification_log_journal,
                         sync_funcs as tg)
...
def notification_methodists_homeworks_unaccepted(tz=3, today=False) -> None:
    def collect_methodists_info():
        ...
        if today:
            ...
            query["dt__date__lte"] = (datetime.datetime.now() -
        ...
        homeworks = HomeworkLog.objects.filter(
            **query
                      ...
                      "homework__lesson__learningphases__"
                      "learningplan__metodist__id",
                      "homework__lesson__learningphases__"
                      "learningplan__metodist__telegram__tg_id",
                      "homework__lesson__learningphases__"
                      "learningplan__metodist__telegram__access_token",
                      named=True)
        methodists_info_ = {}
        for hw in homeworks:
            if hw.homework__lesson__learningphases__learningplan__metodist__id not in methodists_info_:
                methodists_info_[
                    hw.homework__lesson__learningphases__learningplan__metodist__id
                ] = {
                    "hw": [],
                    "tg_id":
                        hw.homework__lesson__learningphases__learningplan__metodist__telegram__tg_id,
                    "access_token":
                        hw.homework__lesson__learningphases__learningplan__metodist__telegram__access_token
                }
            methodists_info_[
                hw.homework__lesson__learningphases__learningplan__metodist__id
            ]["hw"].append(hw)
    ...
    for methodist in methodists_info:
        ...
        message_text += "\n".join(
            [f'{hw_info.homework__name} '
             f'({hw_info.homework__listener__first_name} '
        ...
        if len(methodists_info[methodist]["hw"]) > 10:
            message_text += (f'\n\nИ ещё '
                             f'{len(methodists_info[methodist]["hw"]) - 10}')
        ...
        result = tg.send_tg_message_sync(
            ...
            message=message_text,
            reply_markup=get_homework_notification_menu_buttons(
                access_token=[methodist]["access_token"]
            )
        )
