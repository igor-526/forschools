...
from material.serializers import FileSerializer, MaterialListSerializer
...
from .models import (Homework,
                     HomeworkGroups,
                     HomeworkLog)
from .permissions import (get_can_accept_log_permission,
                          get_delete_log_permission)
...
class HomeworkSerializer(serializers.ModelSerializer):
    ...
    def get_lesson_info(self, obj: Homework):
        if self.hw_lesson:
            ...
            if self.hw_learning_plan and get_can_see_plan(
                    self.context.get('request'),
                    self.hw_learning_plan,
                    self.hw_lesson
            ):
                result['plan'] = {
    ...
    def get_actions(self, obj: Homework):
        ...
        def get_send_actions(last_log_: HomeworkLog,
            ...
            if (obj.teacher == self.context.get("request").user or
                    (plan_ and
                     plan_.metodist == self.context.get("request").user) or
                    (plan_ and
        ...
        def get_cancel_actions(last_log_: HomeworkLog,
                               plan_: LearningPlan) -> List[str]:
            ...
            if (obj.teacher == self.context.get("request").user or
                    (plan_ and plan_.metodist == self.context.get(
                        "request").user) or
                    self.context.get("request").user.groups.filter(
                        name__in=["Admin"]
                    ).exists()):
                return ["cancel"]
            return []

        def get_edit_actions(last_log_: HomeworkLog,
                             plan_: LearningPlan) -> List[str]:
    ...
    def get_send_tg(self, obj: Homework):
        ...
        if (("Teacher" in user_groups or
            ...
            if (self.hw_learning_plan and
                    self.hw_learning_plan.metodist and
                    (self.request.user != self.hw_learning_plan.metodist and
                     self.hw_learning_plan.metodist.get_has_tg())):
...
class HomeworkListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "name", "description", "teacher",
                  "for_curator", "listener", "status", "lesson_info",
                  "assigned", "color", "admin_comment"]
...
class HomeworkLogListSerializer(serializers.ModelSerializer):
    ...
    def get_deletable_logs(self, all_logs, user_groups):
        ...
        for log in all_logs:
            ...
            if log.status == last_logs[-1].status:
                ...
                break
        if (("Metodist" in user_groups or "Admin" in user_groups) and
                last_logs[0].status in [4, 5, 6]):
            return last_logs
        result = []
        for log in last_logs:
            if log.user == self.context.get("request").user:
    ...
    def create(self, validated_data):
        ...
        if plan and plan.metodist:
            if not (self.context.get("request").user.groups.filter(
                    name="Admin").exists() or
