...
from .permissions import (CanSeeEventJournalMixin,
                          get_editable_perm,
                          get_secretinfo_perm)
...
class DeactivateUserAPIView(LoginRequiredMixin, APIView):
    ...
    def patch(self, request, *args, **kwargs):
        try:
            user = NewUser.objects.get(pk=kwargs.get('pk'))
            if (get_secretinfo_perm(request.user, user) and
                    user.get_info_deactivate().get('can_deactivate') is True):
...
class UserListAPIView(LoginRequiredMixin, ListAPIView):
    ...
    def filter_queryset_fields(self, queryset):
        ...
        return queryset.filter(**query).exclude(
            **exclude_query).order_by(*order_query)
