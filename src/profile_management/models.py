...
class NewUser(AbstractUser):
    ...
    last_activity = models.DateTimeField(verbose_name='Последняя активность',
                                         null=False,
                                         auto_now_add=True)
    last_activity_type = models.IntegerField(
        verbose_name="Тип последней активности",
        choices=PROFILE_LAST_ACTIVITY_TYPE_CHOICES,
        null=False,
        default=2
    )
    tg_code = models.IntegerField(
        verbose_name='Код для присоединения Telegram',
        null=True,
        blank=True
    )
    ...
    work_experience = models.CharField(verbose_name='Опыт работы',
                                       ...
                                       blank=True,
                                       max_length=1000)
    listener_category = models.ManyToManyField(
        ListenerCategory,
        verbose_name='Категории учеников',
        blank=True
    )
    private_lessons = models.BooleanField(
        verbose_name='Индивидуальные занятия',
        null=True,
        blank=True
    )
    ...
    engagement_channel = models.ForeignKey(EngagementChannel,
                                           ...
                                           null=True,
                                           on_delete=models.SET_NULL)
    setting_notifications_email = models.BooleanField(
        verbose_name="НАСТРОЙКА: Получать уведомления на email",
        null=False,
        default=True
    )
    ...
    def set_engagement_channel(self, eng_ch, can_create=False):
        if eng_ch is None:
            ...
            if eng_ch != "":
                if can_create:
                    eng_channel = EngagementChannel.objects.get_or_create(
                        name=eng_ch
                    )
                    self.engagement_channel = eng_channel[0]
                else:
                    try:
                        eng_channel = EngagementChannel.objects.get(
                            name=eng_ch
                        )
    ...
    def get_info_deactivate(self):
        ...
        if group_chats:
            ...
            result["group_chats"] = [
                {"id": item.id, "name": item.name} for
                item in group_chats
            ]
        group_chats_admin = self.group_chats_admin.all()
        if group_chats_admin:
            result["can_deactivate"] = False
            result["group_chats_admin"] = [
                {"id": item.id, "name": item.name} for
                item in group_chats_admin
            ]
        homework_teacher = self.homeworks.exclude(
            log_set__status__in=[4, 6, 7]
        )
        if homework_teacher:
            result["can_deactivate"] = False
            result["homework_teacher"] = [
                {"id": item.id, "name": item.name} for
                item in homework_teacher
            ]
        homework_listener = self.homework_set.exclude(
            log_set__status__in=[4, 6, 7]
        )
        if homework_listener:
            result["can_deactivate"] = False
            result["homework_listener"] = [
                {"id": item.id, "name": item.name} for
                item in homework_listener
            ]
        ...
        if plan_curator:
            ...
            result["plan_curator"] = [
                {"id": item.id, "name": item.name} for
                item in plan_curator
            ]
        plan_listeners = self.plan_listeners.all()
        listed_plans = list(filter(
            lambda plan: plan.get_is_closed(), plan_listeners
        ))
        if listed_plans:
            result["can_deactivate"] = False
            result["plan_listeners"] = [
                {"id": item.id, "name": item.name} for
                item in listed_plans
            ]
        plan_methodist = self.plan_metodist.all()
        listed_plans = list(filter(
            lambda plan: plan.get_is_closed(), plan_methodist
        ))
        if listed_plans:
            result["can_deactivate"] = False
            result["plan_methodist"] = [
                {"id": item.id, "name": item.name} for
                item in listed_plans
            ]
        plan_teacher = self.plan_teacher.all()
        listed_plans = list(filter(
            lambda plan: plan.get_is_closed(), plan_teacher
        ))
        if listed_plans:
            result["can_deactivate"] = False
            result["plan_teacher"] = [
        ...
        listed_plans = list(filter(
            lambda plan: plan.get_is_closed(), plan_hw_teacher
        ))
        if listed_plans:
            result["can_deactivate"] = False
            result["plan_hw_teacher"] = [
                {"id": item.id, "name": item.name} for
                item in listed_plans
            ]
        ...
        if lesson_replace_teacher:
            result["can_deactivate"] = False
            result["lesson_replace_teacher"] = [
                {"id": item.id, "name": item.name} for
                item in lesson_replace_teacher
            ]
        return result
    ...
    def get_unread_messages_count(self, sender=None, sender_type=0):
        ...
        return self.message_receiver.filter(
            **query['filter']
        ).exclude(**query['exclude']).count()
    ...
    def get_last_message_date(self):
        ...
        if s_message and r_message:
            return s_message.date if s_message.date > r_message.date \
                else r_message.date
        if s_message:
    ...
    def get_has_tg(self):
        return (self.telegram_allowed_user.exists() or
                self.telegram_allowed_parent.exists())
...
class Telegram(models.Model):
    ...
    async def aget_users_for_chat(self):
        ...
        async def get_user_chat_info(user: dict,
                                     self_user_type: int,
                                     user_type: int):
            info = {
                "id": user['id'],
