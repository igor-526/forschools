...
from .serializers import (UserLogsHWLogsSerializer,
                          UserLogsLessonReviewSerializer,
                          UserLogsMessageSerializer,
                          UserLogsSerializer,
                          UserLogsTGBotJournalSerializer)
...
class UserLogsActionsAPIView(LoginRequiredMixin, APIView):
    ...
    def set_plan_info_and_all_users_ids(self, plan_id: int):
        ...
        if self.lp.teacher:
            self.plan_info["teacher"] = NewUserNameOnlyListSerializer(
                self.lp.teacher, many=False).data
            teacher_id = self.lp.teacher.id
        else:
            teacher_id = None

        if self.lp.default_hw_teacher:
            self.plan_info["default_hw_teacher"] = (
                NewUserNameOnlyListSerializer(
                    self.lp.default_hw_teacher, many=False).data)
            hw_teacher_id = self.lp.default_hw_teacher.id
        else:
            hw_teacher_id = None

        if self.lp.metodist:
            self.plan_info["methodist"] = NewUserNameOnlyListSerializer(
                self.lp.metodist, many=False).data
            methodist_id = self.lp.metodist.id
        else:
            methodist_id = None
        listeners = self.lp.listeners.all()
        if listeners:
            self.plan_info["listeners"] = NewUserNameOnlyListSerializer(
                listeners, many=True).data
        listeners = [listener.id for listener in listeners] \
            if listeners else []
        curators = self.lp.curators.all()
        if curators:
            self.plan_info["curators"] = NewUserNameOnlyListSerializer(
                curators, many=True).data
        curators = [c.id for c in curators] if curators else []
        all_users = [teacher_id, hw_teacher_id,
                     methodist_id, *listeners, *curators]
    ...
    def get_hw_logs(self, plan_id: int):
        ...
        if plan_id:
            query_hw_logs[('homework__lesson__'
                           'learningphases__learningplan__id')] = plan_id
        ...
        hw_logs_ser = UserLogsHWLogsSerializer(
            hw_logs_queryset,
            many=True,
    ...
    def get(self, request, *args, **kwargs):
        ...
        hw_logs = self.get_hw_logs(kwargs.get('plan_pk')) \
            if request.query_params.get('homeworks') == 'true' else []
        tg_journal_notes = self.get_tg_journal_notes() \
            if request.query_params.get('tg_journal') == 'true' else []
        messages = self.get_messages() \
            if request.query_params.get('messages') == 'true' else []
        lessons = self.get_lessons(kwargs.get('plan_pk')) \
            if request.query_params.get('lessons') == 'true' else []
        plans = self.get_plans(kwargs.get('plan_pk')) \
            if request.query_params.get('plans') == 'true' else []
        offset = int(request.query_params.get('offset')) \
            if request.query_params.get('offset') else 0
        all_notes = [*hw_logs, *tg_journal_notes,
                     *messages, *lessons, *plans][offset:offset + 50]
        all_notes = sorted(all_notes, key=itemgetter("date"), reverse=True)
        return Response(data={"plan_info": self.plan_info, "logs": all_notes},
...
class UserLogsMessagesUsersAPIVIew(LoginRequiredMixin, APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if learning_plan.teacher:
            users.append(
                self.get_user_info(learning_plan.teacher, "Преподаватель")
            )
        if (learning_plan.default_hw_teacher and
                learning_plan.default_hw_teacher != learning_plan.teacher):
            users.append(self.get_user_info(
                learning_plan.default_hw_teacher, "Проверяющий ДЗ"
            ))
        if learning_plan.metodist:
            users.append(
                self.get_user_info(learning_plan.metodist, "Методист")
            )
        users.extend([self.get_user_info(user, "Куратор") for
                      user in learning_plan.curators.all()])
        users.extend([self.get_user_info(user, "Ученик") for
                      user in learning_plan.listeners.all()])
        if selected_first_user is not None:
            ...
            users = list(filter(
                lambda user: user.get("id") != selected_first_user, users
            ))
...
class UserLogsMessagesChatAPIVIew(LoginRequiredMixin, APIView):
    ...
    def get_messages(self, query):
        return [{
            ...
            "date": message.date,
            "type": ("receiver"
                     if self.selected_first_user == message.sender.id
                     else "sender"),
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(
            data={"messages": self.get_messages(self.get_queryset()),
                  **self.get_users_info()},
            status=status.HTTP_200_OK
        )
...
class UserLogsMessagesByTagAPIVIew(LoginRequiredMixin, APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(data=self.get_messages_info(query),
                        status=status.HTTP_200_OK)
