...
from material.serializers import FileLogSerializer, MaterialLogSerializer
...
class UserLogsHWLogsSerializer(serializers.ModelSerializer):
    ...
    def get_title(self, obj):
        ...
        if lesson:
            totalstr += (f' к занятию "{lesson.name}" от '
...
class UserLogsTGBotJournalSerializer(serializers.ModelSerializer):
    ...
    def get_title(self, obj):
        role = get_role_ru(get_role_from_plan(
            self.context['plan'], obj.recipient), "g", True)
    ...
    def get_content(self, obj):
        ...
        if obj.initiator:
            list_data.append({
                "name": "Инициатор",
                "val": f'<a target="_blanl" href="/profile/'
                       f'{obj.initiator.id}">{obj.initiator.first_name} '
                       f'{obj.initiator.last_name}</a>'
            })
        list_data.append({
            "name": "Статус",
...
class UserLogsMessageSerializer(serializers.ModelSerializer):
    ...
    def get_title(self, obj):
        role_sender = get_role_ru(get_role_from_plan(
            self.context['plan'], obj.sender))
        role_receiver = get_role_ru(get_role_from_plan(
            self.context['plan'], obj.receiver), "d", True)
...
class UserLogsLessonReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["title", "content", "user",
                  "date", "buttons", "files", "color"]

    def get_title(self, obj):
        lesson = obj.lesson_set.first()
        return (f"Занятие {lesson.name} от "
                f"{lesson.date.strftime('%d.%m.%Y')} проведено")
