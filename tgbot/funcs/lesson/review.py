...
async def f_lesson_review_set_field(message: Message, state: FSMContext,
    ...
    if len(message.text) > max_length:
        await bot.send_message(chat_id=message.from_user.id,
                               text=f"Длина наименования занятия не может превышать {max_length} символов")
        return False
...
async def f_lesson_review_ask_field(tg_id: int, state: FSMContext, field_name: str,
                                    text_ask: str, text_current: str, set_state: State,
                                    lesson_id: int = None) -> None:
    sd = await state.get_data()
    text = text_ask
    if sd.get('lesson_review') and sd['lesson_review'].get(field_name) is not None:
        text += f"\n\n<b>{text_current}:</b> <code>{sd['lesson_review'][field_name]}</code>"
...
async def f_lesson_review_confirm(message: Message, state: FSMContext) -> None:
    ...
    if sd.get('lesson_review') is None:
        ...
        await send_menu(message.from_user.id, state)
        return None
    text = "<b>Пожалуйста, проверьте ревью и измените в случае необходимости</b>\n\n"
    if sd['lesson_review'].get('name') is not None:
        text += f"<b>Наименование занятия (УМК)</b>: {sd['lesson_review']['name']}\n"
...
async def f_lesson_review_set_passed(message: Message, state: FSMContext) -> None:
    ...
    async def assing_and_notify():
        homeworks = [_ async for _ in lesson.homeworks.select_related("teacher", "listener").all()]
