...
from tgbot.funcs.homeworks.homework_logs import f_homework_logs_change_log_message, f_homework_logs_change_log_ready
...
@router.callback_query(HomeworkLogEditingCallback.filter(F.action == "feedback_change"))
async def h_homework_log_edit_text(callback: CallbackQuery,
                                   callback_data: HomeworkLogEditingCallback,
                                   state: FSMContext) -> None:
    await f_homework_logs_change_log_message(callback, state, callback_data.hw_log_id)


@router.message(StateFilter(HomeworkLogFSM.edit_log),
                F.text == "Отмена")
async def h_homework_log_cancel(message: types.Message, state: FSMContext) -> None:
    await send_menu(message.from_user.id, state)


@router.message(StateFilter(HomeworkLogFSM.edit_log),
                F.text == "Готово")
async def h_homework_log_ready(message: types.Message, state: FSMContext) -> None:
...
async def h_homework_log_keep_media_group(message: types.Message, state: FSMContext, media_events=[]):
    files_list = []
    comments_list = []
...
@router.message(StateFilter(HomeworkLogFSM.edit_log))
async def h_homework_log_keep_files(message: types.Message, state: FSMContext) -> None:
    messages = [message]
